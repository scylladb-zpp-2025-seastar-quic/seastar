...
private:
    ...
    bool push_read_fragment(temporary_buffer<char> payload) {
        ...
        if (payload_size && _receive_budget && !_receive_budget->try_acquire(payload_size)) {
            ...
            if (_command_runtime) {
                _command_runtime->mark_error(quic_error_code::io, "receive queue limit exceeded");
            }
            return false;
        }
        ...
        if (_command_runtime) {
            _command_runtime->mark_error(quic_error_code::io, "stream read queue is full");
        }
        return false;
    }
...
void connection_state::on_stream_data(stream_id sid, stream_type type, bool peer_initiated, temporary_buffer<char> payload, bool fin) {
    ...
    if (peer_initiated && st->mark_accepted_for_delivery()) {
        if (!_impl->accepted_streams.push(shared_ptr<stream_state>(st)) && _impl->command_runtime) {
            _impl->command_runtime->mark_error(quic_error_code::io, "accepted stream queue is full");
        }
    }
