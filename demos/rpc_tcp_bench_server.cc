...
// RPC-over-TLS/TCP benchmark server.
//
// Same verbs and stats as the rpc-quic-transport suite, but the RPC layer
// runs over a TLS-on-TCP socket instead of QUIC. This is the apples-to-apples
// baseline for both QUIC RPC suites: same RPC framing/serialization, same
// TLS encryption — only the transport differs (TCP vs QUIC).
//
//   verb 1  echo(sstring) -> sstring   -- returns the received payload
//   verb 2  discard(sstring) -> no_wait -- fire-and-forget sink (TX only)
...
// ---------------------------------------------------------------------------
// Minimal serializer -- matches rpc_demo_quic_v1.cc
// ---------------------------------------------------------------------------
...
static future<> print_stats_loop(unsigned interval_s, abort_source& as) {
    ...
    try {
        while (true) {
            ...
            uint64_t calls = g_stats.calls_echo + g_stats.calls_discard + g_stats.calls_ping;
            ...
            fmt::print(
                "[server] calls/s={:.0f} echo={} discard={} ping={} rx={:.1f} MB/s tx={:.1f} MB/s\n",
                call_rate,
                g_stats.calls_echo, g_stats.calls_discard, g_stats.calls_ping,
                rx_mb_s, tx_mb_s);
            std::cout.flush();
        }
...
int main(int argc, char** argv) {
    ...
    return app.run(argc, argv, [&app]() -> future<int> {
        ...
        try {
            ...
            co_await certs->set_x509_key_file(crt, key, tls::x509_crt_format::PEM);

            ::listen_options lo;
            lo.reuse_address = true;
            auto ss = tls::listen(certs, parse_ipv6_address(address, port), lo);
            ...
            fmt::print("[server] RPC/TLS-TCP bench server listening on [{}]:{}\n", address, port);
