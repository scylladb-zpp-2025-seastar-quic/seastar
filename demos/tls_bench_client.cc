...
static future<> run_latency_connection(
    ...
    co_await write_mode_header(output, traffic_mode::latency);
...
static future<throughput_result> run_throughput_shard(
    ...
    auto credentials = co_await make_client_credentials(ca_file);
    throughput_result result;
    ...
    for (unsigned i = 0; i < connections; ++i) {
        connection_futures.push_back(run_connection_throughput(
            credentials, remote, server_name, mode, msg, deadline, flush_messages, result));
    }
...
static future<latency_samples> run_latency_shard(
    ...
    connection_futures.reserve(connections);
    for (unsigned i = 0; i < connections; ++i) {
        connection_futures.push_back(run_latency_connection(
            credentials, remote, server_name, msg, deadline, samples));
    }
...
int main(int argc, char** argv) {
    ...
    return app.run(argc, argv, [&app]() -> future<int> {
        ...
        try {
            ...
            fmt::print(
                "[client] mode={} stream-type={} shards={} conns={} msg={}B dur={}s flush-msgs={}\n",
                bench_mode,
            ...
            if (bench_mode == "throughput") {
                ...
                for (unsigned shard = 0; shard < this_smp_shard_count(); ++shard) {
                    const auto local_connections = connections_on_shard(connections, shard);
                    shard_futures.push_back(smp::submit_to(shard,
                        [ca_file, remote, server_name, traffic, msg, deadline,
                         flush_messages, local_connections] () mutable {
                            return run_throughput_shard(std::move(ca_file), remote,
                                std::move(server_name), traffic, std::move(msg), deadline,
                                flush_messages, local_connections);
                        }));
                }
                ...
                for (unsigned shard = 0; shard < this_smp_shard_count(); ++shard) {
                    const auto local_connections = connections_on_shard(connections, shard);
                    shard_futures.push_back(smp::submit_to(shard,
                        [ca_file, remote, server_name, msg, deadline,
                         local_connections] () mutable {
                            return run_latency_shard(std::move(ca_file), remote,
                                std::move(server_name), std::move(msg), deadline,
                                local_connections);
                        }));
                }
