...
// RPC-over-TLS/TCP benchmark client. Same client logic as the rpc-quic-transport
// suite, but the socket factory hands the RPC protocol a TLS-wrapped TCP
// socket instead of a QUIC socket. Apples-to-apples baseline against both
// QUIC RPC suites.
//
// Two benchmark modes (--mode):
//
//   throughput  Opens --connections RPC clients. Each one fires --concurrency
//               in-flight RPC calls of size --message-size for --duration
//               seconds.  Two call types:
//                 --call-type echo     server returns the payload (TX + RX)
//                 --call-type discard  fire-and-forget no_wait (TX only)
//
//   latency     Opens --connections RPC clients, issues sequential calls on
//               each (--concurrency parallel streams of sequential calls),
//               records per-call RTT and reports p50/p95/p99/p99.9/max/mean.
//               Uses the echo verb (payload round-trip) by default, or the
//               ping verb with --call-type ping (zero-payload round-trip).
//
// Run rpc_bench_server on the other end before starting this client.
...
// ---------------------------------------------------------------------------
// Minimal serializer -- matches rpc_demo_quic_v1.cc / rpc_bench_server.cc
// ---------------------------------------------------------------------------
...
static call_type_t parse_call_type(const std::string& s) {
    ...
    throw std::runtime_error("Unknown call-type '" + s + "': expected echo, discard or ping");
}
...
struct throughput_result {
    ...
    uint64_t bytes_received = 0;  // echo only
...
// ---------------------------------------------------------------------------
// Per-worker coroutines (one worker = one in-flight chain on a client)
// ---------------------------------------------------------------------------

using rpc_proto = rpc::protocol<serializer>;

// Throughput: issue calls back-to-back until deadline.
...
static future<> run_throughput_worker(
    ...
    try {
        while (bm_clock::now() < deadline) {
            co_await call_fn();
            if (ctype == call_type_t::echo) {
                result.bytes_sent     += msg_size;
                result.bytes_received += msg_size;
            } else if (ctype == call_type_t::discard) {
                result.bytes_sent += msg_size;
            }
            ++result.calls_done;
        }
...
static future<> run_connection_throughput(
    ...
    auto sock = tls::socket(std::move(creds), std::move(server_name));
    rpc::client_options co;
    auto client = std::make_unique<rpc_proto::client>(proto, co, std::move(sock), addr);
    ...
    for (int i = 0; i < concurrency; ++i) {
        if (ctype == call_type_t::echo) {
            ...
            futs.push_back(run_throughput_worker(std::move(call_fn), payload, ctype, deadline, result));
        } else {
            auto call_fn = [&, p = payload]() mutable {
                return discard(*client, p).discard_result();
            };
            futs.push_back(run_throughput_worker(std::move(call_fn), payload, ctype, deadline, result));
        }
    }
...
static void print_throughput(
    ...
    fmt::print("\n=== RPC/TLS-TCP Throughput Benchmark: call-type={} ===\n", call_type_name(ctype));
...
static void print_latency(
    ...
    fmt::print("\n=== RPC/TLS-TCP Latency Benchmark: call-type={} ===\n", call_type_name(ctype));
    fmt::print("  Connections: {}  Concurrency/conn: {}  Msg size: {} B  Elapsed: {:.2f} s\n",
        n_conns, concurrency, msg_size, elapsed_s);
...
int main(int argc, char** argv) {
    ...
    app.add_options()
        ("address", bpo::value<std::string>()->default_value("::1"),
         "Server IPv6 address")
        ("port", bpo::value<uint16_t>()->default_value(10001),
         "Server TCP port")
        ...
        ("call-type", bpo::value<std::string>()->default_value("echo"),
         "RPC call type: echo (round-trip, TX+RX), discard (no_wait, TX only), "
         "ping (zero-payload round-trip, latency only)")
    ...
    return app.run(argc, argv, [&app]() -> future<int> {
        ...
        try {
            ...
            if (mode == "latency" && ctype == call_type_t::discard) {
                throw std::runtime_error(
                    "call-type discard cannot be timed (no reply); use echo or ping");
            }
