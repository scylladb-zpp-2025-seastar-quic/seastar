...
public:
    ...
    int shutdown_stream_write(stream_id, application_error_code) override {
        return 0;
    }

    int shutdown_stream_read(stream_id, application_error_code) override {
        return 0;
    }
    ...
    void on_stream_write_closed(stream_id) override {
    }
    ...
    std::optional<quic_internal::transport_command> pop_blocked_open_stream(stream_type) override {
        return std::nullopt;
    }

    bool blocked_open_stream_retry_pending(stream_type) const noexcept override {
        return false;
    }
