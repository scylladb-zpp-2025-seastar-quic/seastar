...
class fake_connection_actor final {
public:
    bool active = true;
    ...
    bool stop_after_terminal = false;
    size_t rx_events = 0;
    size_t commands = 0;
    size_t wait_calls = 0;
    size_t stop_calls = 0;
    ...
    bool actor_has_pending_work() const noexcept {
        return stop_requested || terminal || rx_events || commands || retry_pending || tick_pending;
    }
...
future<> run_connection_actor(fake_connection_actor& actor) {
...
SEASTAR_TEST_CASE(test_quic_receive_budget_failure_marks_runtime_error) {
    return seastar::async([] {
        connection_options options;
        options.max_pending_receive_bytes = 4;
        auto runtime = quic_internal::make_command_runtime(options);
        ...
        BOOST_REQUIRE(runtime->transport_failed());
        BOOST_REQUIRE(runtime->transport_error() == quic_error::io);
        BOOST_REQUIRE(!runtime->transport_error_detail().empty());
        auto accepted = engine->accept_stream().get();
...
SEASTAR_TEST_CASE(test_quic_actor_stop_request_has_highest_priority) {
    return seastar::async([] {
        fake_connection_actor actor;
        actor.stop_requested = true;
        actor.terminal = true;
        actor.rx_events = 1;
        actor.commands = 1;
        actor.retry_pending = true;
        actor.tick_pending = true;

        run_connection_actor(actor).get();

        BOOST_REQUIRE_EQUAL(actor.log, "S");
        BOOST_REQUIRE_EQUAL(actor.stop_calls, 1);
        BOOST_REQUIRE_EQUAL(actor.terminal_calls, 0);
        BOOST_REQUIRE_EQUAL(actor.rx_calls, 0);
        BOOST_REQUIRE_EQUAL(actor.command_calls, 0);
        BOOST_REQUIRE_EQUAL(actor.retry_calls, 0);
        BOOST_REQUIRE_EQUAL(actor.timer_calls, 0);
    });
}

SEASTAR_TEST_CASE(test_quic_actor_transport_terminal_preempts_rx_and_commands) {
    return seastar::async([] {
        fake_connection_actor actor;
        actor.terminal = true;
        actor.stop_after_terminal = true;
        actor.rx_events = 1;
        actor.commands = 1;
        actor.retry_pending = true;
        actor.tick_pending = true;
        ...
        BOOST_REQUIRE_EQUAL(actor.log, "T");
        BOOST_REQUIRE_EQUAL(actor.terminal_calls, 1);
        BOOST_REQUIRE_EQUAL(actor.rx_calls, 0);
        BOOST_REQUIRE_EQUAL(actor.command_calls, 0);
        BOOST_REQUIRE_EQUAL(actor.retry_calls, 0);
        BOOST_REQUIRE_EQUAL(actor.timer_calls, 0);
...
SEASTAR_TEST_CASE(test_quic_actor_processes_work_in_expected_order) {
    return seastar::async([] {
        fake_connection_actor actor;
        actor.rx_events = 2;
        actor.commands = 2;
        ...
        BOOST_REQUIRE_EQUAL(actor.log, "RRCCBK");
        BOOST_REQUIRE_EQUAL(actor.wait_calls, 1);
        BOOST_REQUIRE_EQUAL(actor.retry_calls, 1);
        BOOST_REQUIRE_EQUAL(actor.timer_calls, 1);
    });
}

SEASTAR_TEST_CASE(test_quic_actor_batch_limit_yields_between_large_rx_batches) {
    return seastar::async([] {
        fake_connection_actor actor;
        actor.rx_events = 70;
        actor.commands = 1;
        ...
        BOOST_REQUIRE_EQUAL(actor.rx_calls, 70);
        BOOST_REQUIRE_EQUAL(actor.command_calls, 1);
        BOOST_REQUIRE_EQUAL(actor.wait_calls, 1);
        BOOST_REQUIRE_EQUAL(actor.log.size(), 71);
        BOOST_REQUIRE_EQUAL(actor.log.substr(0, 64), std::string(64, 'R'));
        BOOST_REQUIRE_EQUAL(actor.log[64], 'C');
        BOOST_REQUIRE_EQUAL(actor.log.substr(65), std::string(6, 'R'));
    });
}
